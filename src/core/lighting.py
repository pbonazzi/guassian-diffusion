...
def diffuse(normals, color, direction) -> torch.Tensor:
    ...
    normals, color, direction = convert_to_tensors_and_broadcast(normals, color, direction, device=normals.device)
...
def specular(
    ...
    acc_color = torch.sum(color * torch.pow(alpha, shininess)[..., None], dim=1)
    return acc_color
...
class DirectionalLights(lighting.DirectionalLights):
    ...
    def diffuse(self, normals, points=None) -> torch.Tensor:
        ...
        return diffuse( normals=normals, color=self.diffuse_color, direction=self.direction)

    def specular(self, normals, points, camera_position, shininess) -> torch.Tensor:
        return specular(points=points,normals=normals, color=self.specular_color,direction=self.direction, camera_position=camera_position, shininess=shininess,)
...
class PointLights(lighting.PointLights):
    ...
    def diffuse(self, normals, points, verbose=False) -> torch.Tensor:


        location, points = convert_to_tensors_and_broadcast(self.location, points, device=self.device)
        batch, L = location.shape[:2]
        location = location.view((batch, L) + (1,) * (points.ndim - 2) + (3,))
