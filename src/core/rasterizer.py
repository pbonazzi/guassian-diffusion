...
class PointsRasterizationSettings:
    ...
    def __init__(
        ...
        frnn_radius: Optional[float] = 0.2,
    ):
...
class SurfaceSplattingRasterizer(PointsRasterizer):
    """
    ...
    def transform_normals(self, point_clouds, **kwargs):
        ...
        # calling normals_padded() will also create points_padded, so must enable grad
    ...
    def _filter_backface_points(self, point_clouds, **kwargs):
        ...
        if features_padded is not None:
            ...
            new_point_clouds = point_clouds.__class__(points=points_list, normals=normals_list, features=features_list)
        else:
            new_point_clouds = point_clouds.__class__(points=points_list, normals=normals_list)
    ...
    def _filter_points_with_invalid_depth(self, point_clouds, **kwargs):
        ...
        mask_packed = ops3d.padded_to_packed(mask.float(), point_clouds.cloud_to_packed_first_idx(), lengths.sum().item()).bool()
        if torch.all(mask_packed):
            return point_clouds, mask_packed
    ...
    def filter_renderable(self, point_clouds, point_clouds_filter=None, **kwargs):
        ...
        if point_clouds_filter is not None:  # activation filter
            point_clouds_filter.set_filter(visibility=torch.full((batch_size, max_P), False, dtype=torch.bool, device=point_clouds_filter.device))
            point_clouds = point_clouds_filter.filter_with(point_clouds, ('activation',))
        ...
        point_clouds, valid_depth_mask = self._filter_points_with_invalid_depth(point_clouds, **kwargs)
        ...
        if raster_settings["backface_culling"]:
            point_clouds, frontface_mask = self._filter_backface_points(point_clouds, **kwargs)
            valid_depth_mask[valid_depth_mask.clone()] = frontface_mask

        return point_clouds, valid_depth_mask

    def _compute_anisotropic_Vrk(self, pointclouds):
        """
        ...
        curvatures, local_frame = estimate_pointcloud_local_coord_frames(pointclouds, neighborhood_size=8, disambiguate_directions=False)

        local_frame = ops3d.padded_to_packed(local_frame.reshape(local_frame.shape[:2] + (-1,)), pointclouds.cloud_to_packed_first_idx(),num_points.sum().item())
        curvatures = ops3d.padded_to_packed(curvatures, pointclouds.cloud_to_packed_first_idx(), num_points.sum().item())
        ...
        Vr = local_frame @ torch.diag_embed(curvatures) @ local_frame.transpose(1, 2)
        return Vr, local_frame.transpose(1, 2)

    def _compute_global_Vrk(self, pointclouds, knn_points, refresh=True, **kwargs):
        """
        ...
        if not refresh and self._Vrk_h is not None:
            ...
            if self.raster_settings["frnn_radius"] <= 0:
                sq_dist, _, _ = ops3d.knn_points(pts_world, pts_world, num_points_per_cloud, num_points_per_cloud, K=knn_points)
            else:
                sq_dist, _, _, _ = frnn.frnn_grid_points(pts_world, pts_world, num_points_per_cloud, num_points_per_cloud, K=knn_points, r=self.raster_settings.frnn_radius)
        ...
        u0 = F.normalize(torch.cross(normals, normals + torch.rand_like(normals)), dim=-1)
        u1 = F.normalize(torch.cross(normals, u0), dim=-1)
        Sk = torch.stack([u0, u1], dim=1)
        Vrk = Vrk_h.view(-1, 1, 1) * Sk.transpose(1, 2) @ Sk
        return Vrk, Sk

    def _compute_isotropic_Vrk(self, pointclouds, knn_points, refresh=True, **kwargs):
        """
        ...
        """
        if not refresh and self._Vrk_h is not None and pointclouds.num_points_per_cloud().sum() == self._Vrk_h.shape[0]:
            pass
        else:
            with torch.autograd.enable_grad():
                pts_world = pointclouds.points_padded()
    ...
    def _compute_variance_and_detMk(self, pointclouds, **kwargs):
        ...
        if raster_settings["Vrk_invariant"]:
            Vrk, Sk = self._compute_global_Vrk(pointclouds, raster_settings["knn_points"], **kwargs)
        elif raster_settings["Vrk_isotropic"]:
            Vrk, Sk = self._compute_isotropic_Vrk(pointclouds,  raster_settings["knn_points"], **kwargs)
        else:
        ...
        variance = Vk + raster_settings["antialiasing_sigma"] * \
            ops3d.eyes(2, totalP, device=Vk.device, dtype=Vk.dtype) * (pixel_size**2)
    ...
    def _compute_WJk(self, point_clouds, **kwargs):
        ...
        denom = pts_packed_hom[:, None, :] @ gather_batch_to_packed(M44[..., 3:], point_clouds.packed_to_cloud_idx())  # P,1,1
        denom = denom.view(-1)
        ...
        Wk = gather_batch_to_packed(M44[..., :2], point_clouds.packed_to_cloud_idx())
    ...
    def _get_ellipse_axis_aligned_radius(self, cutoffC, ellipseParams, min_radii=-float('inf'), max_radii=float('inf')):
        """
    ...
    def _get_per_point_info(self, pointclouds, **kwargs):
        ...
        radii, cutoff_thres = self._get_ellipse_axis_aligned_radius(cutoff_thres, ellipseParams, min_radii=pixel_size, max_radii=20 * pixel_size)
    ...
    def _empty_fragments(self, batch_size, **kwargs):
        ...
        idx = torch.full((batch_size, S, S, P), -1, dtype=torch.long, device=self.device)
        zbuf = torch.full((batch_size, S, S, P), -1.0, dtype=torch.float, device=self.device)
        qvalue_map = torch.full((batch_size, S, S, P), -1.0, dtype=torch.float, device=self.device)
        occ_map = torch.full((batch_size, S, S), 0,  dtype=torch.float, device=self.device)
    ...
    def forward(self, point_clouds, point_clouds_filter=None, **kwargs) -> PointFragments:
        ...
        point_clouds_filtered, mask_filtered = self.filter_renderable(point_clouds, point_clouds_filter, **kwargs)
        ...
        with torch.autograd.no_grad():
            per_point_info = self._get_per_point_info(point_clouds_filtered, **kwargs)

        _tmp = point_clouds_filtered.points_padded()
        if _tmp.requires_grad:
            _tmp.register_hook(lambda x: _check_grad(x, 'transform'))
        pcls_screen = self.transform(point_clouds_filtered, eps=1e-17, **kwargs)

        idx, zbuf, qvalue_map, occ_map = rasterize_elliptical_points(
            pcls_screen=pcls_screen,
            ellipse_params=per_point_info["ellipse_params"],
            cutoff_threshold=per_point_info['cutoff_threshold'],
            radii=per_point_info["radii"],
        ...
        if idx.isnan().any() or zbuf.isnan().any() or qvalue_map.isnan().any() or occ_map.isnan().any() :
            pdb.set_trace()

        # compute weight: scalar*exp(-0.5Q)
        frag_scaler = gather_with_neg_idx(per_point_info['scaler'], 0, idx.view(-1).long())
        frag_scaler = frag_scaler.view_as(qvalue_map)

        fragments = PointFragments(idx=idx, zbuf=zbuf, qvalue=qvalue_map, scaler=frag_scaler, occupancy=occ_map)

        # returns (P,) boolean mask for visibility
        visibility_mask = get_per_point_visibility_mask(point_clouds_filtered, fragments)
        mask_filtered[mask_filtered.clone()] = visibility_mask

        if point_clouds_filter is not None:
            original_visibility_mask = ops3d.packed_to_padded(mask_filtered.float(), point_clouds.cloud_to_packed_first_idx(), max_P).bool()
...
def rasterize_elliptical_points(pcls_screen, 
                                ellipse_params,
                                cutoff_threshold, 
                                radii,
                                depth_merging_threshold: float,
                                image_size: int,
    ...
    1. check_inside(pixel, point) by a) axis-aligned radii and the ellipse funciton f(x)<cutoff_threshold
    ...
    Args:
        ...
        ellipse_params (tensor): (N, 3) ellipse parameters per splat, the (a,b,c) in ax^2 + bxy + cy^2
        radii (tensor): (N, 2) axis-aligned radius
    Returns:
        PointFragments containing [B,C,H,W] maps of idx (C=1), f(x) (C=1), zbuf (C=1)
    """
    ...
    if bin_size != 0:
        ...
        if num_bins >= kMaxPointsPerBin:
            raise ValueError("bin_size too small, number of bins must be less than %d; got %d" % (kMaxPointsPerBin, num_bins) )

    if max_points_per_bin is None:
        max_points_per_bin = int(max(10000, num_points_per_cloud.max()/5))
...
class EllipticalRasterizer(autograd.Function):
    ...
    def forward(ctx, 
        ...
        zbuf0 = zbuf[..., 0].clone()
        ctx.save_for_backward(pts_screen, ellipse_param, cutoff_threshold, radii, idx, zbuf0, cloud_to_packed_first_idx, num_points_per_cloud)
    ...
    def backward(ctx, idx_grad, zbuf_grad, qvalue_grad, occ_grad):
        ...
        pts_screen, ellipse_param, cutoff_threshold, radii, idx, zbuf0, cloud_to_packed_first_idx, num_points_per_cloud = ctx.saved_tensors
        depth_merging_threshold = ctx.depth_merging_threshold
        radii_s = ctx.radii_backward_scaler

        grad_ellipse_param = None
        ...
        pts_visibility = torch.full((pts_screen.shape[0],), False, dtype=torch.bool, device=pts_screen.device)
        ...
        num_points_per_cloud = torch.stack([x.sum() for x in torch.split( pts_visibility, num_points_per_cloud.tolist(), dim=0)])
        cloud_to_packed_first_idx = num_points_2_cloud_to_packed_first_idx( num_points_per_cloud)
        ...
        frnn._C.counting_sort_cuda(pts_padded_2D, num_points_per_cloud,pc_grid_cell,pc_grid_idx, pc_grid_off,
            points_sorted,      # (N,P,2)
        ...
        grad_sorted = _C._splat_points_occ_fast_cuda_backward(  points_sorted, 
                                                                radii_sorted, 
                                                                search_radius, 
                                                                occ_grad, 
                                                                num_points_per_cloud, 
        ...
        grads = (grads_input, grad_ellipse_param, grad_cutoff_thres, grad_radii, grad_cloud_to_packed_first_idx,
                 grad_num_points_per_cloud,  grad_depth_merging_thres, grad_image_size, grad_points_per_pixel,
                 grad_bin_size, grad_max_points_per_bin, grad_radii_s)
