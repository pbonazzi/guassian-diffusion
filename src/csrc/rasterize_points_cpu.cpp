    ...
    const int points_per_pixel)
{
  ...
  for (int n = 0; n < N; ++n)
  {
    ...
    // in the point cloud.
    const int point_start_idx = cloud_to_packed_first_idx[n].item().to<int32_t>();
    const int point_stop_idx = (point_start_idx + num_points_per_cloud[n].item().to<int32_t>());
    ...
    for (int yi = 0; yi < S; ++yi)
    {
      ...
      {
        // Reverse the order of xi so that +X is pointing to the left in the image.
