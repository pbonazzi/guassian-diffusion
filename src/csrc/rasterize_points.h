...
//
// Args:
//  points: Tensor of shape (P, 3) giving (packed) positions for points in all N pointclouds in the batch where P is the total
//          number of points in the batch across all pointclouds. These points are expected to be in NDC coordinates in the range [-1, 1].
//  ellipse_params: Tensor of shape (P, 3) giving the quadratic coefficients for each splatted point i.e. the (a,b,c) in ax^2 + bxy + cy^2
...
//  cloud_to_packed_first_idx: LongTensor of shape (N) giving the index in  points_packed of the first point in each pointcloud  in the batch where N is the batch size.
//  num_points_per_cloud: LongTensor of shape (N) giving the number of points for each pointcloud in the batch.
...
//  idxs: int32 Tensor of shape (N, S, S, K) giving the indices of the  closest K points along the z-axis for each pixel, padded with -1 for
//        pixels hit by fewer than K points. The indices refer to points in points packed i.e a tensor of shape (P, 3) representing the flattened points for all pointclouds in the batch.
//  zbuf: float32 Tensor of shape (N, S, S, K) giving the depth of each closest point for each pixel.
//  qvalue: float32 Tensor of shape (N, S, S, K) giving the exponent to compute the composition weight exp(-Qvalue)
    ...
    const int points_per_pixel)
{
  if (points.is_cuda())
  {
    ...
    return RasterizePointsNaiveCuda( points, ellipse_params, cutoff_thres, radii, cloud_to_packed_first_idx, num_points_per_cloud,  depth_merging_thres,image_size,  points_per_pixel);
#else
  ...
  {
    return RasterizePointsNaiveCpu(  points, ellipse_params, cutoff_thres, radii, cloud_to_packed_first_idx, num_points_per_cloud, depth_merging_thres,  image_size, points_per_pixel);
  }
}
...
//  cloud_to_packed_first_idx: LongTensor of shape (N) giving the index in points_packed of the first point in each pointcloud
