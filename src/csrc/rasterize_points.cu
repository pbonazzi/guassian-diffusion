...
namespace
{
    ...
    struct Pix
    {
        ...
        float qvalue;   // the value of the ellipse_function Q=(a dx^2+b dxdy+c dy^2) of the points corresponding to idx map. Note: EWA ~ exp(-0.5Q)
    };
    ...
    // point with index p and batch index n. One of the inputs is a list (q) which contains Pixel structs 
    // with the indices of the points which intersect
    ...
    // This code is shared between RasterizePointsNaiveCudaKernel and RasterizePointsFineCudaKernel.
    ...
    float *occupancies)  // (N, S, S)
{
    ...
    for (int i = tid; i < N * S * S; i += num_threads)
    {
        ...
        {
            CheckPixelInsidePoint(  points, radii, ellipse_params, p_idx, cutoff_thres[p_idx], q_size, q_max_z, q_max_idx, q, xf, yf, K);
        }
    ...
    int *bin_points)
{
    ...
    for (int chunk = blockIdx.x; chunk < num_chunks; chunk += gridDim.x)
    {
        ...
        for (int byx = threadIdx.x; byx < num_bins * num_bins; byx += blockDim.x)
        {
            ...
            const int points_per_bin_idx =  batch_idx * num_bins * num_bins + by * num_bins + bx;
            ...
            // out to bin_points.
            int next_idx = batch_idx * num_bins * num_bins * M + by * num_bins * M + bx * M + start;
            for (int p = 0; p < chunk_size; ++p)
            {
                if (binmask.get(by, bx, p))
                {
                    // TODO: Throw an error if next_idx >= M -- this means that we got more than max_points_per_bin in this bin
    ...
    const int max_points_per_bin)
{
    ...
    at::TensorArg points_t{points, "points", 1},
        ...
        cloud_to_packed_first_idx_t{cloud_to_packed_first_idx, "cloud_to_packed_first_idx", 3},
        num_points_per_cloud_t{num_points_per_cloud, "num_points_per_cloud", 4};
    at::CheckedFrom c = "RasterizePointsCoarseCuda";
    at::checkAllSameGPU( c, {points_t, radii_t, cloud_to_packed_first_idx_t, num_points_per_cloud_t});
    ...
    RasterizePointsCoarseCudaKernel<<<blocks, threads, shared_size, stream>>>(
        ...
        cloud_to_packed_first_idx.contiguous().data_ptr<int64_t>(),
        num_points_per_cloud.contiguous().data_ptr<int64_t>(),
        N,  P,  image_size,  bin_size,  chunk_size, M,
    ...
    const int points_per_pixel)
{
    ...
    RasterizePointsFineCudaKernel<<<blocks, threads, 0, stream>>>(
        ...
        radii.contiguous().data_ptr<float>(),
        bin_points.contiguous().data_ptr<int32_t>(),
        depth_merging_thres, bin_size,  N,  B,   M,  S,  K,
        point_idxs.contiguous().data_ptr<int32_t>(),
        zbuf.contiguous().data_ptr<float>(),
...
)
{
    ...
    for (int i = tid; i < num_pixels; i += num_threads)
    {
        ...
        {
            // Using the batch index of the thread get the start and stop indices for the points.
            const int64_t point_start_idx = cloud_to_packed_first_idx[n];
            const int64_t point_stop_idx = point_start_idx + num_points_per_cloud[n];
    ...
    const float depth_merging_thres)
{
    ...
    at::TensorArg points_t{points, "points", 1},
        ...
        cloud_to_packed_first_idx_t{cloud_to_packed_first_idx, "cloud_to_packed_first_idx", 4},
        num_points_per_cloud_t{num_points_per_cloud, "num_points_per_cloud", 5};
    at::CheckedFrom c = "RasterizePointsOccBackwardCuda";
    at::checkAllSameGPU(c, {points_t, radii_t, grad_occ_t, cloud_to_packed_first_idx_t, num_points_per_cloud_t});
...
{
    // Check inputs are on the same device, size checks are done in rasterize_points.h
    at::TensorArg idx_t{idx, "idx", 1}, zbuf_grad_t{zbuf_grad, "zbuf_grad", 2};
    at::CheckedFrom c = "RasterizeZbufBackwardCuda";
    at::checkAllSameGPU(c, {idx_t, zbuf_grad_t});
