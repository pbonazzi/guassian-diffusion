...
class FilterOptions(BaseOptions):
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--flip-normal', action="store_false", dest="backfaceCulling", help='flip normal wrt view direction')
        parser.add_argument('--pix2pix', type=str, default="render_PCA_resnet", help="model name for pix2pix")
        parser.add_argument('--recursiveFiltering', action="store_true", help="apply the same filter on the output")
        ...
        parser.set_defaults(steps=[19, 1], learningRates=[2000, 1], projectionRadius=0.1, projectionWeight=0.05,
                            repulsionRadius=0.03, repulsionWeight=0.05,
                            repulsionFreq=1, projectionFreq=1, averageWeight=0.01,
