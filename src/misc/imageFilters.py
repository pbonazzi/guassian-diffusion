...
def L0Smooth(images, lmd=0.05):
    ...
    for idx, I in enumerate(images):
        ...
        # compute FFT denominator (second part only)
        FI = torch.fft.fft2(I, dim=(0, 1))
        dx = torch.zeros((I.shape[0], I.shape[1]))  # gradient along x direction
        dy = torch.zeros((I.shape[0], I.shape[1]))  # gradient along y direction

        dx[dx.shape[0]//2, dx.shape[1]//2-1:dx.shape[1]//2+1] = torch.tensor([-1, 1])
        dy[dy.shape[0]//2-1:dy.shape[0]//2+1, dy.shape[1]//2] = torch.tensor([-1, 1])


        denominator_second = torch.conj(torch.fft.fft2(dx))*torch.fft.fft2(dx) + torch.conj(torch.fft.fft2(dy))*torch.fft.fft2(dy)
        den_second = torch.tile(denominator_second.unsqueeze(2).clone(), [1, 1, I.shape[2]]).to(I.device)
        ...
        for iter in range(numIter):
            # solve hp, vp
            hp = torch.cat((S[:, 1:], S[:, :1]), axis=1) - S
            vp = torch.cat((S[1:, :], S[:1, :]), axis=0) - S
            
            if len(I.shape) == 3:
                zeroIdx = torch.sum(hp**2+vp**2, axis=2) < lmd/beta
            else:
            ...
            # solve S
            hv = torch.cat((hp[:, -1:], hp[:, :-1]), axis=1) - hp + torch.cat((vp[-1:, :], vp[:-1, :])) - vp
            S = torch.real(torch.fft.ifft2((FI + (beta*torch.fft.fft2(hv))) / (1+beta*den_second)))
            ...
            if S.isnan().sum().item() > 0:
                print("Stopping L0 Smooth because of NAN")
                break
        ...
        results.append(saved_S)

    return torch.stack(results)

def Pix2PixInitialize():
    ...
    opt.checkpoints_dir = '/home/bpietro/thesis/DSS/checkpoints/Pix2Pix'
    # opt.name = 'render_PCA_resnet'
    opt.name = 'latest_net_G'
    opt.epoch = "latest"
...
def Img2ImgDiffusionInitialize(model_path):
    from dependencies.SD.pipe_denoising import StableDiffusionImg2ImgDenoisingPipeline
    return StableDiffusionImg2ImgDenoisingPipeline.from_pretrained(model_path,torch_dtype=torch.float32,)


def ImgL0Smooth(images, lmd=0.05):
    results = []
    for idx, TensorI in enumerate(images):
        betaMax = 1e5
        beta = 0.1
        betaRate = 2.0
        numIter = 40
        isTensor = False
        if isinstance(TensorI, torch.Tensor):
            device = TensorI.device
            I = TensorI.detach().cpu().numpy()
            isTensor = True
