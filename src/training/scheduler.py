...
class TrainerScheduler(object):
    ...
    def step(self, trainer, it):
        if self.steps_dss_backward_radii > 0 and hasattr(trainer.model, 'renderer'):
            ...
            gamma = self.gamma_dss_backward_radii ** ( it // self.steps_dss_backward_radii)
            
            raster_settings["radii_backward_scaler"] = max(self.init_dss_backward_radii * gamma, self.limit_dss_backward_radii)
            if old_backward_scaler != raster_settings["radii_backward_scaler"]:
                logger_py.info('Updated radii_backward_scaler: {} -> {}'.format(old_backward_scaler, raster_settings["radii_backward_scaler"]))
