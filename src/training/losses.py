...
from src.utils.mathHelper import eps_denom, estimate_pointcloud_normals
from src import get_debugging_mode, get_debugging_tensor
from src import get_logger
logger_py = get_logger(__name__)


def grad_loss(output, gt, verbose=False):
    def one_grad(shift, img):
        ox = output[img, shift:] - output[img, :-shift]
        oy = output[img, :, shift:] - output[img, :, :-shift]
        gx = gt[img, shift:] - gt[img, :-shift]
        gy = gt[img, :, shift:] - gt[img, :, :-shift]
        loss = (ox - gx).abs().mean() + (oy - gy).abs().mean()
        return loss

    loss = 0
    for i in range(gt.shape[0]):
        loss += (one_grad(1, i) + one_grad(2, i) + one_grad(3, i)) / 3.

    return loss/gt.shape[0]

def vivi_loss(output, gt, verbose=False):
    def one_grad(shift, img):
        ox = output[img, shift:] - output[img, :-shift]
        oy = output[img, :, shift:] - output[img, :, :-shift]
        gx = gt[img, shift:] - gt[img, :-shift]
        gy = gt[img, :, shift:] - gt[img, :, :-shift]
        loss = (ox).abs().mean() - (gx).abs().mean() + (oy).abs().mean() - (gy).abs().mean()
        return loss
...
class BaseLoss(nn.Module):
    ...
    def _reduce(self, loss, reduction=None):
        ...
        if reduction == 'sum':
            ...
            raise ValueError( 'Invalid reduction method ({})'.format(self.reduction))
        return loss
...
def check_nan(vec):
    return vec.isnan().sum() + vec.isinf().sum()

class SemanticConsistencyLoss(BaseLoss):
...
class NormalLoss(BaseLoss):
    ...
    def compute(self, pointclouds: PointClouds3D, *args, neighborhood_size=None, **kwargs):
        ...
        normals_ref = padded_to_packed(normals_ref, pointclouds.cloud_to_packed_first_idx(), num_points.sum().item())

        return -F.cosine_similarity(normals_packed, normals_ref).abs()
...
class SurfaceLoss(BaseLoss):
    ...
    def _build_knn(self, point_clouds):
        ...
        knn_result = ops.knn_points(points_padded, points_padded, lengths, lengths, K=self.knn_k, return_nn=True)
        self.knn_mask = torch.full(knn_result.idx.shape, False, dtype=torch.bool, device=points_padded.device)
        # valid knn result
        for b in range(self.knn_mask.shape[0]):
            self.knn_mask[b, :lengths[b], :min(self.knn_k, lengths[b].item())] = True
            assert(torch.all(knn_result.dists[b][~self.knn_mask[b]] == 0))
        self.knn_tree = KNN( knn=knn_result.knn[:, :, 1:, :], dists=knn_result.dists[:, :, 1:], idx=knn_result.idx[:, :, 1:])
            ...
            try:
                ...
                if len(point_clouds) != reliable_normals_mask.shape[0]:
                    if len(point_clouds) == 1 and reliable_normals_mask.shape[0] > 1:
                        reliable_normals_mask = reliable_normals_mask.any(dim=0, keepdim=True)
                    # reset visible points normals to its original ones
                    elif normals_denoised.shape[0] > 1 and normals.shape[0] > 1:
                        reliable_normals_mask = reliable_normals_mask.repeat(normals_denoised.shape[0], 1)
                        
                    else:
                        ValueError("Incompatible point clouds {} and mask {}".format(len(point_clouds), reliable_normals_mask.shape))

                normals_denoised[reliable_normals_mask ==  1] = normals[reliable_normals_mask == 1]
            
            except KeyError as e:
                pass

        normals_denoised_packed = ops.padded_to_packed(normals_denoised, point_clouds.cloud_to_packed_first_idx(), P_total)
    ...
    def get_normal_w(self, point_clouds: PointClouds3D, normals: Optional[torch.Tensor] = None, **kwargs):
        ...
        """
        self.sharpness_sigma = kwargs.get('sharpness_sigma', self.sharpness_sigma)
    ...
    def get_spatial_w(self, point_clouds: PointClouds3D, points: Optional[torch.Tensor] = None, **kwargs):
        ...
        w = torch.exp(-torch.sum(deltap * deltap, dim=-1) * inv_sigma_spatial[0] * self.filter_scale)
        return w
    ...
    def get_phi(self, point_clouds, **kwargs):
        ...
        w = 1 - self.knn_tree.dists /  eps_denom(h)
        w[w < 0] = 0
        w = w * w
        w = w * w
        return w
...
class ProjectionLoss(SurfaceLoss):
    ...
    def compute(self, point_clouds: PointClouds3D, points_filter=None, rebuild_knn=False, **kwargs):
        ...
        with torch.autograd.no_grad():
            ...
            point_clouds = self._denoise_normals(point_clouds, phi, points_filter, inplace=False)

            # compute wn and wr
            normal_w = self.get_normal_w(point_clouds, **kwargs)

            # visibility weight
            if points_filter.visibility.unsqueeze(-1).shape[0] != self.knn_tree.idx.shape[0]:
                visibility_nb = ops.knn_gather(points_filter.visibility.unsqueeze(-1).repeat(self.knn_tree.idx.shape[0], 1, 1), self.knn_tree.idx, lengths)
            else:
            ...
            if point_clouds.normals_padded().shape[0] != self.knn_tree.idx.shape[0]:
                knn_normals = ops.knn_gather(point_clouds.normals_padded().repeat(self.knn_tree.idx.shape[0], 1, 1), self.knn_tree.idx, lengths)
            else:
                knn_normals = ops.knn_gather(point_clouds.normals_padded(), self.knn_tree.idx, lengths)



        sdf = torch.sum((self.knn_tree.knn.detach() - points.unsqueeze(-2)) * knn_normals, dim=-1)

        # convert everything to packed
        weights = ops.padded_to_packed( weights, point_clouds.cloud_to_packed_first_idx(), P_total)
        sdf = ops.padded_to_packed( sdf, point_clouds.cloud_to_packed_first_idx(), P_total)

        distance_to_face = sdf*sdf

        # compute weighted signed distance to surface
        loss = torch.sum( weights * distance_to_face, dim=-1) / eps_denom(torch.sum(weights, dim=-1))
...
class RepulsionLoss(SurfaceLoss):
    """
    Intend to compute the repulsion term in src Eq(12)~Eq(15)
    without SVD
    """
    ...
    def get_density_w(self, point_clouds: PointClouds3D, points: Optional[torch.Tensor], **kwargs):
        ...
        if points is None:
            with torch.autograd.enable_grad():
                points = point_clouds.points_padded()
        deltap = self.knn_tree.knn - points[:, :, None, :]
        w = 1 + torch.sum(torch.exp(-torch.sum(deltap * deltap, dim=-1)  * inv_sigma_spatial), dim=-1)
        return w
    ...
    def compute(self, point_clouds: PointClouds3D, points_filter=None, rebuild_knn=True, **kwargs):
        ...
        knn_normals = ops.knn_gather(point_clouds.normals_padded(), self.knn_tree.idx, lengths)
        pts_diff_proj = knn_diff -  (knn_diff * knn_normals).sum(dim=-1, keepdim=True) * knn_normals
        ...
        # convert everything to packed
        weights = ops.padded_to_packed(weights, point_clouds.cloud_to_packed_first_idx(), P_total)
        pts_diff_proj = ops.padded_to_packed(pts_diff_proj.contiguous().view(pts_diff_proj.shape[0], pts_diff_proj.shape[1], -1), point_clouds.cloud_to_packed_first_idx(), P_total).view(P_total, -1, 3)
        density_w = ops.padded_to_packed( density_w, point_clouds.cloud_to_packed_first_idx(), P_total)

        # we want to maximize this, so negative sign
        repel_vec = torch.sum( pts_diff_proj * weights.unsqueeze(-1), dim=1) / eps_denom(torch.sum(weights, dim=1).unsqueeze(-1))
