...
from src.core.lighting import PointLights
...
def slice_dict_data(d: dict, idx: List):
    """ slice all values in a dict to take the idx
    """        
    ...
    for k, v in d.items():
        if k == "input":
            ...
            slice_dict(v, idx)
        elif isinstance(v, FoVPerspectiveCameras):
            if isinstance(v.R[idx], np.ndarray):
                d[k].R = torch.tensor(v.R[idx]).unsqueeze(0)
                d[k].T = torch.tensor(v.T[idx]).unsqueeze(0)
            else:                
                d[k].R = v.R[idx].unsqueeze(0)
                d[k].T = v.T[idx].unsqueeze(0)
            d[k]._N = 1

        elif isinstance(v, PointLights):
            d[k].location = v.location[idx].unsqueeze(0)
            d[k].ambient_color = v.ambient_color[idx].unsqueeze(0)
            d[k].diffuse_color = v.diffuse_color[idx].unsqueeze(0)
            d[k].specular_color = v.specular_color[idx].unsqueeze(0)
            d[k]._N = 1
        else:
            d[k] = v[idx]
    return d
...
def get_tensor_values(tensor: torch.Tensor, p: torch.Tensor, grid_sample=True, mode='bilinear',
    ...
    if grid_sample:
        ...
        values = torch.nn.functional.grid_sample(
            tensor, p, mode=mode, align_corners=True, padding_mode='reflection')
        # (B,c,N)
