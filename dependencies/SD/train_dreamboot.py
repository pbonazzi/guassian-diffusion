...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if class_data_root is not None:
            ...
            for i in range(len(chair_roots)):
                self.class_data_root = Path(os.path.join(class_data_root, chair_roots[i]))
                if not self.class_data_root.exists():
                    raise ValueError("Instance images root doesn't exists.")
                inst_list = list(Path(self.class_data_root).iterdir())[:-2]
                self.class_images_path = [*self.class_images_path, *inst_list]
