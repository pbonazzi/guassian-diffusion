...
from dependencies.CLIP import clip
...
CLIP_DE_NORMALIZE = torchvision.transforms.Normalize(   mean=[-m / s for m, s in zip(CLIP_MEAN, CLIP_STD)],
                                                        std=[1.0 / s for s in CLIP_STD]
                                                    )
...
def load_vit(name="ViT-B/32", jit=False, device=DEVICE, root: str=os.path.expanduser("~/.cache/clip")):
    ...
    if clip_model_vit is None:
        ...
        clip_model_vit.eval()
        # Sclip_model_vit = torch.nn.DataParallel(CLIPEncoder(clip_model_vit))
...
def test_consistency():
    ...
    image = clip._transform(Image.open("/data/storage/bpietro/datasets/DSS/images/kangaroo8/train/image/000000.png")).unsqueeze(0).to(device)
    text = clip.tokenize(["a kangaroo", "a phone", "a cat"]).to(device)

    with torch.no_grad():
        logits_per_image, _ = clip_model_vit(image, text)
